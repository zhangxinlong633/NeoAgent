...
11. Daemon 多轮 — `neo daemon` / `-D` / `--daemon` / `--socket`；交互式 `User>` / `neo>` + Markdown；UTF-8 退格（`IUTF8`）；默认仅内存；可选 `-D -S id` 挂载落盘会话（多 id 加载、写回第一个）
...
3. 具名会话 / 混合 — `./neo -S id` / `-S a,b`（落盘；可恢复续聊优先用这个）
4. JSON / 文件 — `./neo -j "…"` / `./neo -o FILE "…"` / `./neo -j -o FILE "…"`
5. DAG — `./neo dag run <名>` / `./neo run <名|"目标">` / `./neo plan "目标"`
6. 多轮 REPL — `./neo -D`（内存）或 `./neo -D -S id`（挂载落盘）/ `--socket PATH`
